...
app.include_router(usuarios.router)  # registro/login de alumnos y tutores + rutas protegidas por rol
...
# Nota de seguridad: storage/uploads/ ya NO se sirve como estático público.
# Ahí conviven los apuntes (que sí son públicos, pero bajan por su propio
# endpoint protegido /api/apuntes/{id}/descargar) con los títulos/DNI que
# suben los tutores al postularse (que NO deben ser públicos — ver
# GET /api/admin/certificaciones/{user_id}/documento, solo para moderadores).
