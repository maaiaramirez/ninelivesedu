...
def mis_reservas_alumno(user=Depends(require_role("student"))):
    rows = exec_all(
        """SELECT r.id, r.estado AS estado_reserva, r.fecha, r.modalidad,
...
def aprobar_reserva(reserva_id: str, user=Depends(require_role("teacher"))):
    ...
    run("UPDATE reservas SET estado = 'confirmed' WHERE id = ?", (reserva_id,))
    ...
    resultado = {"success": True, "message": "Reserva aprobada.", "cupoLleno": False, "pin": None}
