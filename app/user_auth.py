...
def require_role(*roles: str):
    ...
    Para el rol 'teacher' también revalida que siga aprobado en cada pedido
    (no solo al momento del login) — así un moderador puede revocar el
    acceso de un tutor sin esperar a que su sesión expire sola.
    ...
    def dependency(request: Request):
        ...
        if user["role"] == "teacher" and user["validation_status"] != "approved":
            raise HTTPException(403, "Tu perfil de tutor ya no está aprobado. Contactá a un moderador.")
        return user
    return dependency
