...
async def crear_apunte(
    ...
    if archivo is not None:
        ...
        import time
        filename = f"{int(time.time() * 1000)}-{uuid.uuid4().hex[:8]}.{SAFE_EXT_BY_MIME[archivo.content_type]}"
    ...
    ext = SAFE_EXT_BY_MIME.get(archivo.content_type, "pdf") if archivo else "pdf"
